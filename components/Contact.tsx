...
export default function Contact() {
  ...
  return (
    <View style={{width: width, height: height, backgroundColor: "#1c93ba"}}>
      <Header />
      <View style={{shadowColor: 'black', shadowOffset: {width: 4, height: 3}, borderWidth: 3, borderColor: 'black', borderRadius: 30, marginLeft: 10, marginRight: 10, backgroundColor: "white"}}>
        <Text style={{fontSize: 20, marginLeft: 10, marginTop: 10}}>Write a message</Text>
        <TextInput multiline value={message} onChangeText={setMessage} style={{marginLeft: 10, marginRight: 10, marginTop: 3, height: height * 0.4}}/>
        <Text>Your Contact</Text>
        <TextInput value={email} onChangeText={setEmail} style={{marginLeft: 10, marginRight: 10}}/>
        <Pressable onHoverIn={() => setIsSendHover(true)} onHoverOut={() => {setIsSendHover(false)}} onPress={() => {loadSendMessage()}} style={{backgroundColor: isSendHover ? "#d3d3d3":"white", shadowColor: 'black', shadowOffset: {width: 4, height: 3}, borderWidth: 3, borderColor: 'black', borderRadius: 30, padding: 20, paddingLeft: width * 0.3, paddingRight: width * 0.3, marginLeft: 'auto', marginRight: 'auto'}}>
          <Text>Send</Text>
        </Pressable>
      </View>
